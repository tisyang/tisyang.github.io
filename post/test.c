...
#define LTS_CFG_CONTROL_LIST(X) \
    X(STR, type, CFG_STR_MAXCHARS, "T1", "Lts type", true) \
    X(STR, sn, CFG_STR_MAXCHARS, "000000", "sn number", true) \
    ...
    X(FLOAT, move_speed, 20.0, "move speed deg/s", false) \
    X(FLOAT, scan_start_angle, -50.0, "abs scan start angle", false) \
    X(FLOAT, scan_stop_angle, 50.0, "abs scan stop angle", false) \
    X(FLOAT, watching_angle, 0.0, "angle pos in wathcing mode", false)

#define LIDAR_ROT_DEFAULT ((double[]){ 0, 0, -1, -1, 0, 0, 0, 1, 0 })

#define LTS_CFG_LIDAR_LIST(X) \
    X(STR, type, CFG_STR_MAXCHARS, "H1", "lidar type", true) \
    X(STR, ip, CFG_STR_MAXCHARS, "192.168.0.240", "lidar ip addr", false) \
    X(INT, port, 2111, "lidar port", false) \
    X(FLOAT, p_offset_hori, 0.0, "offset of hori from center", true) \
    X(FLOAT, p_offset_vert, 0.0, "offset of vert from center", true) \
    X(FLOAT_N, p_rot, 9, LIDAR_ROT_DEFAULT, "rot matrix", true)

#define LTS_CFG_CAMERA_LIST(X) \
    X(STR, type, CFG_STR_MAXCHARS, "IMX415", "camera type", false) \
    ...
    X(FLOAT, fps, 1.0, "camera capture fps control", false)
